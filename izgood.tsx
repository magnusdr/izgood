...
type ValidationRuleTuple = [string, (value: any, args: any) => string | undefined, string?, any?]
...
function izgood(formdata: FormData, rules: Rules): FormError[] | undefined {
    ...
    for (let i = 0; i < rules.length; i++) {
        ...
        let err_msg = rule.whats_wrong(formdata.get(rule.name), rule.args)
        if(err_msg) {
            errors.push({
                name: rule.name,
...
function NotGood({ name, errors, className, ...restProps } : NotGoodProps) {
    if(errors) {
        ...
        if(my_err) {
            return <div className={`notgood ${className}`} {...restProps}>
                {my_err.message}
            </div>
        }
    }
...
export function useNotGood(rules: Rules): [(d: FormData) => boolean, typeof NotGood] {
    ...
    return [validate, function({ name, ...restProps } : NotGoodProps) {
        return <NotGood 
            name={name} 
            errors={errors}
            {...restProps} />
...
export function izNotEmpty(value: any, args: any): string | undefined {
    ...
    const stripped_value = value && typeof value === "string" ? value.trim() : value

    return value == null || stripped_value.length === 0 ? "This field cannot be empty" : undefined
}
